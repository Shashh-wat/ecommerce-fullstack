...
# Ensure ucp_server directory is in sys.path so 'import config' works
...
# Move products router above others if paths conflict, but they don't seem to.
...
def main(argv: Sequence[str]) -> None:
  ...
  # Defaults are now handled in config.py, so we can be lenient here?
  # Only if we invoke via absl.
  port = config.FLAGS.port or 8182
